...
def validate_evidence(document: Any) -> list[dict[str, str]]:
    ...
    if not isinstance(result, str) or result not in {"pass", "fail", "blocked"}:
    ...
    if not isinstance(criteria, list) or not criteria:
        ...
        for index, item in enumerate(criteria):
            ...
            if not isinstance(status, str) or status not in {"pass", "fail", "deferred"}:
    ...
    if not isinstance(checks, list) or not checks:
        ...
        for index, item in enumerate(checks):
            ...
            if not isinstance(check_type, str) or check_type not in {
                "automated-test",
                "manual-test",
                "review",
                "build",
...
def closeout_report(
    ...
    """Build a stable Closeout Contract report for any parsed JSON value."""

    evidence_object = evidence if isinstance(evidence, dict) else {}
    effective_errors = errors or validate_evidence(evidence)
    raw_criteria = evidence_object.get("acceptance_criteria", [])
    evidence_items = raw_criteria if isinstance(raw_criteria, list) else []
    raw_checks = evidence_object.get("checks", [])
    ...
    checks = [
        ...
        _check(
            "evidence.schema",
            not effective_errors,
            (
                "evidence matches schema"
                if not effective_errors
                else "; ".join(
                    f"{item['path']}: {item['message']}" for item in effective_errors
                )
            ),
        ),
        _check(
            "evidence.story",
            evidence_object.get("story_id") == story.story_id,
            f"evidence story_id is {evidence_object.get('story_id', '')!r}",
        ),
        _check(
            "evidence.result",
            evidence_object.get("result") == "pass",
            f"result is {evidence_object.get('result', '')!r}",
        ),
    ]
    evidence_criteria = {
        item.get("id"): item
    ...
    for item in evidence_checks:
        ...
        failing_checks.append(
            identifier if isinstance(identifier, str) and identifier else "unknown"
        )
...
def apply_closeout(
    ...
    """Apply a Closeout report and classify atomic writer failures.

    A failed writer leaves the caller's original Story untouched. The mutable
    report is converted into a stable ``closeout.write`` failure so CLI callers
    still receive Closeout Contract 1.0 instead of an unstructured I/O error.
    """
    ...
    try:
        ...
        message = "atomic Story write failed"
        report["verdict"] = "fail"
        report["target_state"] = report["current_state"]
        report["checks"] = [
            *report["checks"],
            _check("closeout.write", False, message),
        ]
        report["failures"] = [
            *report["failures"],
            {"code": "closeout.write", "message": message},
        ]
        return False
    report["written"] = True
    return True
