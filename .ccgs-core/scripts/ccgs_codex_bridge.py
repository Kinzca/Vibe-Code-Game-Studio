...
class CodexBridgeError(ValueError):
    """Raised when Codex Bridge inputs or managed targets are unsafe."""
...
def _read_existing(path: Path) -> str | None:
    if not path.exists() and not path.is_symlink():
        return None
    if path.is_symlink():
        raise CodexBridgeError(f"managed target may not be a symbolic link: {path}")
    if not path.is_file():
        raise CodexBridgeError(f"managed target is not a regular file: {path}")
    try:
        return path.read_bytes().decode("utf-8")
    except (OSError, UnicodeDecodeError) as exc:
        raise CodexBridgeError(f"managed target is not valid UTF-8: {path}") from exc


def _load_template(framework: Path, relative: str) -> str:
    path = framework / "templates" / "codex" / relative
    if not path.is_file():
        raise CodexBridgeError(f"Codex template is missing: {path}")
    try:
        return path.read_text(encoding="utf-8", errors="strict")
    except (OSError, UnicodeDecodeError) as exc:
        raise CodexBridgeError(f"Codex template is not valid UTF-8: {path}") from exc
...
def _managed_skill(existing: str | None, rendered: str, relative: str) -> str:
    ...
    if not lines or lines[0] != "---":
        raise CodexBridgeError(f"Codex Skill template lacks frontmatter: {relative}")
    try:
        frontmatter_end = lines.index("---", 1)
    except ValueError as exc:
        raise CodexBridgeError(
            f"Codex Skill template has unclosed frontmatter: {relative}"
        ) from exc
    ...
    if not _is_managed_skill(existing):
        raise CodexBridgeError(
            f"refusing to replace unmanaged Codex Skill: {relative}"
        )
    return desired
...
def build_codex_plan(framework: Path, project: Path, data_dir: str) -> BootstrapPlan:
    ...
    agents_existing = _read_existing(project / AGENTS_TARGET)
    ...
    for relative, template_relative in SKILL_TARGETS.items():
        existing = _read_existing(project / relative)
