...
OBSERVABILITY_EXPORT_COMMAND = "langfuse-export"
ALLOWED_COMMANDS = {
    "doctor",
    "evidence-validate",
    "closeout",
    "qdrant-query",
    "workflow-observe",
    OBSERVABILITY_EXPORT_COMMAND,
}
FORBIDDEN_CMD_CHARS = frozenset("&|<>^%!\"\r\n\0")
DRIVE_RE = re.compile(r"^[A-Za-z]:")
RETRYABLE_PORT_CODES = frozenset({"PORT_ADAPTER_UNAVAILABLE", "PORT_ADAPTER_TIMEOUT"})
...
class RetryPolicy:
    """Bounded retry settings for transport availability failures only."""
...
def _retryable_cli_failure(payload: Any) -> bool:
    if not isinstance(payload, dict) or payload.get("ok") is not False:
        return False
    error = payload.get("error")
    return (
        isinstance(error, dict)
        and error.get("code") in RETRYABLE_PORT_CODES
...
class CcgsCmdRunner:
    ...
    def invoke(self, command: str, arguments: Sequence[str]) -> dict[str, Any]:
        """Run one CLI operation and retry transport availability failures only."""
        ...
        for attempt in range(1, self.policy.max_attempts + 1):
            ...
            try:
                ...
                outcome = "protocol-error" if process.returncode == 0 else "invocation-error"
                attempts.append(
                    {
                        "attempt": attempt,
                        "outcome": outcome,
            ...
            retryable = process.returncode == 3 and _retryable_cli_failure(payload)
            attempts.append(
                {
                    "attempt": attempt,
                    "outcome": (
...
def run_observed_story_closeout(
    ...
    *,
    ...
    if not 1 <= timeout_seconds <= 300:
        raise WindmillAdapterError("observed timeout_seconds must be between 1 and 300")
    ...
    telemetry = runner.invoke(OBSERVABILITY_EXPORT_COMMAND, telemetry_args)
    failures = list(closeout.get("failures", []))
    failures.extend(_invocation_failures(telemetry))
    status = str(closeout["status"])
    return {
        "schema_version": SCHEMA_VERSION,
        "adapter": ADAPTER_NAME,
        "operation": "observed-story-closeout",
