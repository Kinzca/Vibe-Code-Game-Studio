...
class OtelLangfuseExporter:
    ...
    def export_neutral(self, trace: Mapping[str, Any]) -> dict[str, Any]:
        """Export a neutral trace without model input/output or free-text fields."""
        ...
        try:
            ...
            raise LangfuseAdapterError(
                "OpenTelemetry export requires the optional OTLP packages"
            ) from exc

        trace_id = str(trace.get("trace_id", ""))
        span_id = str(trace.get("span_id", ""))
        if not re.fullmatch(r"[0-9a-f]{32}", trace_id) or not re.fullmatch(r"[0-9a-f]{16}", span_id):
            raise LangfuseAdapterError("neutral trace identity is invalid")
        attributes = trace.get("attributes")
        if not isinstance(attributes, dict):
            raise LangfuseAdapterError("neutral trace attributes are invalid")

        trace_id_value = int(trace_id, 16)
        span_id_value = int(span_id, 16)
        ...
        delegate = OTLPSpanExporter(
            endpoint=self.endpoint, headers=self.headers, timeout=self.timeout_seconds,
        )
        recording = RecordingExporter(delegate)
        provider = TracerProvider(
            resource=Resource.create(
                {
                    "service.name": "vibe-workflow",
                    "service.version": ADAPTER_VERSION,
                    "deployment.environment.name": str(attributes.get("vibe.environment", "unknown")),
                }
            ),
            id_generator=FixedIdGenerator(),
        )
        provider.add_span_processor(SimpleSpanProcessor(recording))
        tracer = provider.get_tracer("vibe.langfuse", ADAPTER_VERSION)
        span = tracer.start_span(
            str(trace.get("name", "workflow")),
            start_time=_time_ns(str(trace.get("start_timestamp", ""))),
            attributes=attributes,
        )
        if attributes.get("vibe.status") in {"fail", "error"}:
            span.set_status(Status(StatusCode.ERROR))
        span.end(end_time=_time_ns(str(trace.get("end_timestamp", ""))))
        provider.force_flush()
        success = recording.result == SpanExportResult.SUCCESS
        provider.shutdown()
        if not success:
            raise LangfuseTransportError("Langfuse OTLP exporter did not acknowledge the span")
        return {"trace_sent": True}
...
class LangfuseScoreClient:
    ...
    def _post_score(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            error = f"Langfuse Score HTTP {exc.code}"
        ...
        if len(raw) > MAX_REMOTE_RESPONSE_BYTES:
            raise LangfuseAdapterError("Langfuse Score response exceeds the size limit")
        if not raw:
            return {}
        try:
    ...
    def send_neutral_metrics(self, metrics: Sequence[Mapping[str, Any]]) -> int:
        """Send allowlisted neutral metrics without comments or arbitrary metadata."""
        ...
        for metric in metrics:
            ...
            payload = {
                ...
                "traceId": str(metric.get("trace_id", "")),
                "observationId": str(metric.get("span_id", "")),
