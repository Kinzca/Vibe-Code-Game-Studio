...
class WindmillAdapterRetryTests(unittest.TestCase):
    ...
    def test_retryable_port_json_exit_is_retried_then_succeeds(self) -> None:
        calls = []
        ...
        with materialized_fixture("mature-project") as project:
            result = self._runner(project, executor).invoke(
                "langfuse-export",
                ["--event", "ccgs-data/production/observability/events/event.json", "--dry-run"],
            )
    ...
    def test_exhausted_telemetry_does_not_override_completed_closeout(self) -> None:
        langfuse_attempts = 0

        def executor(command, **kwargs):
            nonlocal langfuse_attempts
            rendered = " ".join(command) if isinstance(command, list) else command
            operation = command[1] if isinstance(command, list) else rendered
            if operation == "qdrant-query":
                payload = {"results": [], "result_count": 0}
            elif operation == "doctor":
                payload = {"data_dir": "ccgs-data"}
            elif operation == "evidence-validate":
                payload = {"valid": True, "errors": []}
            elif operation == "workflow-observe":
                payload = {"event": "ccgs-data/production/observability/events/wm-run-003.json"}
            elif operation == "langfuse-export":
                langfuse_attempts += 1
                payload = {
                    "ok": False,
                    "error": {
                        "code": "PORT_ADAPTER_TIMEOUT",
                        "message": "Observability operation did not complete",
                        "retryable": True,
                        "details": {},
                    },
        ...
        with materialized_fixture("mature-project") as project:
            result = run_observed_story_closeout(
                str(ROOT), str(project), STORY, EVIDENCE,
                "fixture-project", "wm-run-003", "story-001-workflow",
                "fixture-session-001", max_attempts=2, retry_delay_seconds=0,
                executor=executor, platform="posix",
            )

        self.assertEqual(langfuse_attempts, 2)
        self.assertEqual(result["closeout"]["status"], "passed")
        self.assertEqual(result["telemetry"]["status"], "error")
        self.assertEqual(result["status"], "passed")
        self.assertTrue(result["ok"])
        self.assertFalse(result["retryable"])
        self.assertIs(raise_for_windmill(result), result)

    def test_observed_timeout_contract_rejects_values_over_300(self) -> None:
        with materialized_fixture("mature-project") as project:
            with self.assertRaisesRegex(WindmillAdapterError, "between 1 and 300"):
                run_observed_story_closeout(
                    str(ROOT), str(project), STORY, EVIDENCE,
                    "fixture-project", "wm-run-004", "story-001-workflow",
                    "fixture-session-001", timeout_seconds=301,
                    executor=lambda *args, **kwargs: None, platform="posix",
                )
...
class WindmillAdapterSafetyTests(unittest.TestCase):
    ...
    def test_windmill_assets_are_strict_json_compatible_yaml(self) -> None:
        ...
        observed_module = observed_flow["value"]["modules"][0]
        self.assertEqual(
            observed_module["value"]["input_transforms"]["max_attempts"],
            {"type": "static", "value": 3},
        )
        self.assertEqual(
            observed_module["value"]["input_transforms"]["retry_delay_seconds"],
            {"type": "static", "value": 5},
        )
        self.assertNotIn("retry", observed_module)
        self.assertEqual(
            observed_flow["schema"]["properties"]["timeout_seconds"]["maximum"],
            300,
        )
        self.assertEqual(
            observed_meta["schema"]["properties"]["timeout_seconds"]["maximum"],
            300,
        )
        self.assertEqual(
        ...
        for metadata in (check_meta, closeout_meta):
            self.assertIn("data_dir", metadata["schema"]["properties"])
            self.assertIn("project_id", metadata["schema"]["properties"])
            self.assertIn("request_id", metadata["schema"]["properties"])
        self.assertNotIn("apply", closeout_meta["schema"]["properties"])
    ...
    def test_windmill_scripts_do_not_implement_project_workflow_logic(self) -> None:
        ...
        for path in wrappers:
            ...
            if path.name in {"story_check.py", "story_closeout.py"}:
                self.assertIn("ccgs_windmill_port", text)
            else:
