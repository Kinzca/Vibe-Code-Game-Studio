...
class StoryWorkflowTests(unittest.TestCase):
    ...
    def test_non_object_evidence_returns_stable_closeout_failure(self) -> None:
        with materialized_fixture("mature-project") as project:
            set_review(project)
            evidence_path = project / EVIDENCE
            invalid_check_id = json.loads(evidence_path.read_text(encoding="utf-8"))
            ...
            for collection, field in (
                ...
                if collection == "root":
                    ...
                    document["acceptance_criteria"][0][field] = []
                else:
                    document["checks"][0][field] = []
                invalid_scalars.append(document)
            for invalid_root in (
                [],
                None,
                "invalid",
                ...
                *invalid_scalars,
                ...
                before = tree_digest(project)
                process = run_cli(project, "closeout", "--story", STORY, "--dry-run")
                report = json.loads(process.stdout)

                self.assertEqual(process.returncode, 1, process.stderr)
                self.assertEqual("fail", report["verdict"])
                self.assertEqual("review", report["target_state"])
                self.assertIn("evidence.schema", [item["code"] for item in report["failures"]])
                self.assertEqual(tree_digest(project), before)
